...
'''
迁移学习
数据来源  curl http://download.tensorflow.org/example_images/flower_photos.tgz
         tar xzf flower_photos.tgz
         包含5个子文件夹，每个文件夹对应一种花的类别，每种花有734张图片，每张图片都是RGB色彩模式，大小也不相同
         
训练好的模型  wget https://storage.googleapis.com/download.tensorflow.org/models/inception_dec_2015.zip
...
# Inception-v3模型中代表瓶颈层的张量名称。谷歌提供的Inception-v3模型中，这个张量名称就是'pool_3/_reshape:0'
...
# 下载的谷歌训练好的Inception-v3模型文件的目录
MODEL_DIR = '/Users/caoqi/AI/data/inception_dec_2015'

# 下载的谷歌训练好的Inception-v3模型文件名
MODEL_FILE = 'tensorflow_inception_graph.pb'

# 因为一个训练数据会被使用多次，所以可以将原始图像通过Inception-v3模型计算得到的特征向量保存在文件中，免去重复计算
CACHE_DIR = '/Users/caoqi/AI/data/cache/bottleneck'

# 图片数据文件夹。这个文件夹中每个子文件夹对应一类图片
INPUT_DATA = '/Users/caoqi/AI/data/flower_photos'
...
testing_percentage和validation_percentage参数指定来测试数据和验证数据集的大小
'''
def create_image_lists(testing_percentage, validation_percentage):
    # 得到的所有图片都存放在result这个字典里，key为类别的名称，value也是一个字典，字典里存储里所有图片的名称
    result = {}
    # 获取当前目录下所有的子目录
...
def get_image_path(image_lists, image_dir, label_name, index, category):
    ...
    # 最终的地址为数据根目录的地址加上类别文件夹加上图片的名称
    full_path = os.path.join(image_dir, sub_dir, base_name)
    return full_path

'''
这个函数通过类别的名称、所属数据集和图片编号获取经过Inception-v3模型处理之后的特征向量文件地址
'''
...
'''
这个函数获取一张图片经过Inception-v3模型处理之后的特征向量。这个函数会先试图寻找已经计算且保存下来的特征向量
...
def get_or_create_bottleneck(sess, image_lists, label_name, index, category, jpeg_data_tensor, bottleneck_tensor):
    ...
    # 如果这个特征向量文件不存在，则通过Inception-v3模型来计算特征向量，并将就是得到的结果存入文件
...
def main(_):
    ...
    # 读取已经训练好的Inception-v3模型，谷歌训练好的模型保存在来GraphDef Protocol Buffer中，里面保存了每一个节点取值的计算方法和变量的取值
    ...
    # 定义新的神经网络输入，这个输入就是新的图片经过Inception-v3模型前向传播到达瓶颈层的节点取值。可以将这个过程类似的理解为一种特征提取
    bottleneck_input = tf.placeholder(tf.float32, [None, BOTTLENECK_TENSOR_SIZE], name='BottleneckInputPlaceholder')
    # 定义新的标准答案输入
    ground_truth_input = tf.placeholder(tf.float32, [None, n_classes], name='GroundTruthInput')

    # 定义一层全连接层来解决新的图片分类问题，因为训练好的Inception-v3模型已经将原始的图片抽象为了更加容易分类的特征向量了，
